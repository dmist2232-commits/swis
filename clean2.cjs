...
const mIdx = code.indexOf("app.post('/api/menu'");
const sIdx = code.indexOf("// Settings Management");

const before = code.substring(0, mIdx);
const after = code.substring(sIdx);

const fixedMenu = `app.post('/api/menu', async (req, res) => {
  try {
...
fs.writeFileSync('server.ts', before + fixedMenu + after);
